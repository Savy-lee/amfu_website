...
import { PORTFOLIO_ADMIN_EMAIL } from "../lib/actions/portfolio";
...
async function main() {
  ...
  const canonicalAdmin = adminUsers.find((user) => user.email === PORTFOLIO_ADMIN_EMAIL) ?? adminUsers[0] ?? null;
  ...
  const duplicateAdminIds = adminUsers
    .filter((user) => user.id !== canonicalAdmin?.id)
    .map((user) => user.id);

  if (duplicateAdminIds.length > 0) {
    await prisma.contactMessages.deleteMany({ where: { users_id: { in: duplicateAdminIds } } });
    await prisma.newsletterSubscribers.deleteMany({ where: { users_id: { in: duplicateAdminIds } } });
    await prisma.footerInfos.deleteMany({ where: { users_id: { in: duplicateAdminIds } } });
    await prisma.aboutSections.deleteMany({ where: { users_id: { in: duplicateAdminIds } } });
    await prisma.journeyEntrys.deleteMany({ where: { users_id: { in: duplicateAdminIds } } });
    await prisma.skillGroups.deleteMany({ where: { users_id: { in: duplicateAdminIds } } });
    await prisma.services.deleteMany({ where: { users_id: { in: duplicateAdminIds } } });
    await prisma.projects.deleteMany({ where: { users_id: { in: duplicateAdminIds } } });
    await prisma.users.deleteMany({ where: { id: { in: duplicateAdminIds }, role: "admin" } });
  }
  ...
  if (newsletterCount === 0) {
    await prisma.newsletterSubscribers.createMany({
      data: [
        {
          email: "labanonjhonrey@gmail.com",
          users_id: user.id,
        },
      ],
    });
